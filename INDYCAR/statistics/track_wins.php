...
while ($results_championship = $recordset_championship->fetch_assoc()) {
	...
	while ($results_drivers = $recordset_drivers->fetch_assoc()) {
		...
		while ($results_wins = $recordset_wins->fetch_assoc())
		{
			...
			print'<TR>';
			print"<TD align='left' width=150><FONT size='2'><a href='../driver/driver.php?ID=".$driverID."'>".$results_wins['drivers'].'</a></FONT></TD>';
			print'<TD width=75><FONT><b>'.$results_wins['Siege'].' of '.$events.'</b></FONT></TD>';
			print'<TD width=75><FONT><b>'.Round(100 * $results_wins['Siege'] / $events, 2).' % </b></FONT></TD>';
