...
if(isset($_POST["Import"])) {
	...
	$sql_update_race_champ1 = "INSERT INTO championship (Bezeichnung, Saison, RaceID, Kategorie) SELECT 'NASCAR Cup Series', YEAR(races.Datum), races.ID, 1 FROM races LEFT JOIN championship ON championship.RaceID = races.ID WHERE (championship.ID IS NULL) AND (races.ID MOD 10 IN (1,2,3));";
	$result = mysqli_query($database_connection, $sql_update_race_champ1);
	$sql_update_race_champ2 = "INSERT INTO championship (Bezeichnung, Saison, RaceID, Kategorie) SELECT 'NASCAR Nationwide Series', YEAR(races.Datum), races.ID, 1 FROM races LEFT JOIN championship ON championship.RaceID = races.ID WHERE (championship.ID IS NULL) AND (races.ID MOD 10 IN (4,5,6));";
	$result = mysqli_query($database_connection, $sql_update_race_champ2);
	$sql_update_race_champ3 = "INSERT INTO championship (Bezeichnung, Saison, RaceID, Kategorie) SELECT 'NASCAR Truck Series', YEAR(races.Datum), races.ID, 1 FROM races LEFT JOIN championship ON championship.RaceID = races.ID WHERE (championship.ID IS NULL) AND (races.ID MOD 10 IN (7,8,9));";
	$result = mysqli_query($database_connection, $sql_update_race_champ3);
