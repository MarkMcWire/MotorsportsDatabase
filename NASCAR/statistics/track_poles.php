...
while ($results_championship = $recordset_championship->fetch_assoc()) {
	...
	$query_drivers = "SELECT DriverID, COUNT(DISTINCT races.TrackID) AS Poles FROM race_results INNER JOIN races ON races.ID = race_results.RaceID INNER JOIN championship ON championship.RaceID = race_results.RaceID 
	WHERE (championship.Bezeichnung LIKE '".$results_championship['Bezeichnung']."') AND (race_results.Start = 1)
	...
	while ($results_drivers = $recordset_drivers->fetch_assoc()) {
		...
		while ($results_Poles = $recordset_Poles->fetch_assoc())
		{
			...
			while ($result = $recordset->fetch_assoc())
			{
				...
				$query_details = "SELECT MIN(Start) as Poles FROM race_results INNER JOIN races on races.ID = race_results.RaceID INNER JOIN tracks ON tracks.ID = races.TrackID INNER JOIN championship ON championship.RaceID = races.ID
				WHERE (Start > 0) and (tracks.ID = $trackID) and (race_results.DriverID = $driverID) and (championship.Bezeichnung LIKE '".$results_championship['Bezeichnung']."') GROUP BY race_results.DriverID, races.TypeID, tracks.Kennzeichen";
