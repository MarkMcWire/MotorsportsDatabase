...
<?php
include("verbindung.php");
$query0 = "SELECT TT.Bezeichnung, TT.DriverID, drivers.Name, 
	...
	UNION ALL SELECT championship.Bezeichnung, championship.Saison, championship.Kategorie,
	qualification_results.DriverID, NULL AS Finish, 0 AS Rennpunkte, 0 AS Sprintpunkte, 0 AS Stagepunkte, SUM(qualification_points.Punkte) AS Bonuspunkte
	FROM qualification_results LEFT JOIN championship ON championship.RaceID = qualification_results.RaceID 
	LEFT JOIN qualification_points ON qualification_points.Scoring = championship.Qualification_Scoring
	WHERE (championship.Bezeichnung LIKE '".$championship_name."')
	AND ((qualification_points.Saison = championship.Saison) OR (qualification_points.Saison = 0)) AND (qualification_points.Wert = qualification_results.Position)
	GROUP BY championship.Bezeichnung, championship.Saison, championship.Kategorie, qualification_results.RaceID, qualification_results.DriverID
	UNION ALL SELECT championship.Bezeichnung, championship.Saison, championship.Kategorie,
	...
	GROUP BY TT.Bezeichnung, TT.DriverID, drivers.Name
